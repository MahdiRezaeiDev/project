...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount <= 0) {
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            $fileName = uniqid('payment_') . '.' . pathinfo($_FILES['photo']['name'], PATHINFO_EXTENSION);
        ...
        // Store payment
        $result = storePayment($amount, $account, $date, $customer_id, $user_id, $bill_id, $photoPath, $created_at, $updated_at, $factor);
    ...
    try {
        $stmt = PDO_CONNECTION->prepare("
            INSERT INTO factor.payments 
                (bill_id, amount, date, account, customer_id, user_id, photo, created_at, updated_at)
            VALUES 
                (:bill_id, :amount, :date, :account, :customer_id, :user_id, :photo, :created_at, :updated_at)
        ");
